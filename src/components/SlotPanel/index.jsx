...
const SlotPanel = ({ spin, setSpin }) => {
  const { bidAmounts, currency, chooseBid, chosenBalance } =
    useSelector(selectorCurrency);
  const handleSpin = () => {
  ...
  return (
    <div className="slotPanel">
      ...
      <button
        className={`slotPanel__button ${
          spin || chooseBid > chosenBalance.value ? "disabled" : ""
        }`}
        onClick={handleSpin}
        disabled={spin || chooseBid > chosenBalance.value}
      >
        SPIN
      </button>
    </div>
